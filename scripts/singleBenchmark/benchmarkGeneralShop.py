...
tw_wiki = os.environ.get("TW_WIKI")
tw_webshop = os.environ.get("TW_WEBSHOP")
tw_news = os.environ.get("TW_NEWS")

# Validate that required environment variables are set
if not all([tw_wiki, tw_webshop, tw_news]):
    raise ValueError("Missing required TimeWarp environment variables: TW_WIKI, TW_WEBSHOP, and TW_NEWS must be set")
...
test_benchmark.env_args_list = test_benchmark.env_args_list[159:240]
if not test_benchmark.env_args_list:
    raise ValueError("No tasks found in the specified range [159:240]. Check that the benchmark has enough tasks.")
...
study.dir = Path(f"results/{model_name_for_dir}/{args.version}_shopTest")  # Separate directory to avoid mixing with other results
